"""
Cythonised Lebwohl-Lasher code with attempts of using multithreading.
Currently is 'functional' but does not provide speed-up.
"""
...
cdef all_energy(double[:,:] arr, int nmax, int threads):
    cdef int idx
    cdef double enall = 0.0
    cdef int i, j

    for idx in prange(nmax*nmax, nogil=True, num_threads=threads):
        i = idx // nmax
        j = idx % nmax
